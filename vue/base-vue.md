...
`props` 称为情态数据，在各自实例中，一旦出事华北定义好数据类型，基于`vue`是单项数据流，在数据传递时始终不能修改它的数据类型，而且在子组件不允许直接操作传递过来的`props`，可以通过其他间接方式修改。
...
* 原理：`vue-router`用了单页面应用，`vue`单页面应用基于路由与组件，路由设定访问路径，并将路径与组件映射起来，在`vue`单页面应用切换路径，也是在切换组件，页面加载时，不会加载整个页面，而是更新某个指定容器的内容；`vue-router`在实现前端单页面路由使用了两种模式：`hash`模式与`history`模式，根据`mode`参数决定使用哪一种模式。
